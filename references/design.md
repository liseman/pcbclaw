...
## CAD
Use filesystem-safe short project names under ~/pcb-projects. Inspect existing files/editor state; checkpoint before consequential changes. Missing Git identity means timestamped verified backup, not blocked design. Never invent an author or push without user authorization.
...
## Expansion, mechanics and reuse
For devboard-style expansion, audit actual pin use, boot straps, debug/UART, expander versus MCU functions, voltage limits and available power budget. Label shared/special-function signals accurately; never call used MCU pins free. Verify I2C pin order, voltage, pullups, addresses, switched-power/backfeed behavior and connector orientation. State whether headers are fitted or unpopulated. Reuse existing holes where appropriate; verify screw/head/standoff, copper and antenna clearances and export actual holes. Do not impose expansion or mounting changes as defaults on projects that did not request them.

Known-good reference hardware validates that implementation, not every reuse of its parts. Treat accepted reference heritage as selection evidence while preserving documented differences in ratings, MCU, power, firmware and layout. Record firmware status explicitly: absent, source-only, built/tested, flashed or hardware-validated. A compiled binary or clean ERC/DRC never implies a working delivered product.
